...
def get_data():
    ...
    sentiment_tweets = twitter.search(
        q=twitter_handle + "-filter:retweets AND -filter:replies", count=100
    )
...
def get_sentiment():
    ...
    tweets = twitter.search(
        q=search_term + "-filter:retweets AND -filter:replies", count=100
    )
